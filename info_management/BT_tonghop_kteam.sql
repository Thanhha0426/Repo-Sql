...
-- xuat ra thong tin giáo viên của khoa CNTT
SELECT GV.MAGV,GV.HOTEN, BM.TENBM ,K.TENKHOA
FROM dbo.KHOA AS K, dbo.BOMON AS BM,dbo.GIAOVIEN AS GV
WHERE K.MAKHOA = N'CNTT'
AND GV.MABM = BM.MABM AND BM.MAKHOA =K.MAKHOA

-- đếm số lượng giáo viên khoa CNTT
SELECT COUNT(*) FROM dbo.GIAOVIEN AS GV , dbo.KHOA AS K,dbo.BOMON AS BM
WHERE K.MAKHOA = N'CNTT'
AND GV.MABM = BM.MABM AND BM.MAKHOA =K.MAKHOA

-- xuất ra thông tin của gvien và thông tin của đề tài người đó tham gia mà kết quả là đạt
SELECT GV.MAGV, gv.HOTEN, DT.TENDT,TG.KETQUA
FROM dbo.GIAOVIEN AS GV,  dbo.DETAI AS DT, dbo.THAMGIADT AS TG
WHERE GV.MAGV = TG.MAGV
AND TG.MADT = DT.MADT
AND TG.KETQUA = N'đạt'

-- xuất ra thông tin giáo viên và thông tin GVCN của mỗi người đó
SELECT GV1.MAGV, GV1.HOTEN, GV1.PHAI, GV2.HOTEN
FROM dbo.GIAOVIEN AS GV1, dbo.GIAOVIEN AS GV2
WHERE GV1.MAGV = GV2.GVQLCM

SELECT * FROM dbo.GIAOVIEN
WHERE NGSINH < '19730216'

SELECT COUNT(*), TENKHOA FROM dbo.GIAOVIEN, dbo.BOMON, dbo.KHOA
WHERE BOMON.MABM = GIAOVIEN.MABM AND BOMON.MAKHOA = KHOA.MAKHOA
GROUP BY BOMON.MAKHOA, TENKHOA HAVING COUNT(MAGV) >= 
ALL(SELECT COUNT(*) FROM dbo.GIAOVIEN, dbo.BOMON, dbo.KHOA
WHERE BOMON.MABM = GIAOVIEN.MABM AND BOMON.MAKHOA = KHOA.MAKHOA
GROUP BY BOMON.MAKHOA, TENKHOA)